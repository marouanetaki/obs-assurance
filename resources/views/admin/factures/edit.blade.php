    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.factures.update", [$facture->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('dossier') ? 'is-invalid' : '' }}" name="dossier_id" id="dossier_id" required>
                    @foreach($dossiers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('dossier_id') ? old('dossier_id') : $facture->dossier->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('frais_rembourse') ? 'is-invalid' : '' }}" type="number" name="frais_rembourse" id="frais_rembourse" value="{{ old('frais_rembourse', $facture->frais_rembourse) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('created_by') ? 'is-invalid' : '' }}" name="created_by_id" id="created_by_id" required>
                    @foreach($created_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('created_by_id') ? old('created_by_id') : $facture->created_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('mode_paiement') ? 'is-invalid' : '' }}" name="mode_paiement" id="mode_paiement" required>
                    ...
                    @foreach(App\Models\Facture::MODE_PAIEMENT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('mode_paiement', $facture->mode_paiement) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
