...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.dossier.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.dossiers.update", [$dossier->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('beneficiaire') ? 'is-invalid' : '' }}" name="beneficiaire_id" id="beneficiaire_id" required>
                    @foreach($beneficiaires as $id => $entry)
                        <option value="{{ $id }}" {{ (old('beneficiaire_id') ? old('beneficiaire_id') : $dossier->beneficiaire->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_soins') ? 'is-invalid' : '' }}" type="text" name="date_soins" id="date_soins" value="{{ old('date_soins', $dossier->date_soins) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('frais_consultation') ? 'is-invalid' : '' }}" type="number" name="frais_consultation" id="frais_consultation" value="{{ old('frais_consultation', $dossier->frais_consultation) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('frais_analyse') ? 'is-invalid' : '' }}" type="number" name="frais_analyse" id="frais_analyse" value="{{ old('frais_analyse', $dossier->frais_analyse) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('frais_pharmacie') ? 'is-invalid' : '' }}" type="number" name="frais_pharmacie" id="frais_pharmacie" value="{{ old('frais_pharmacie', $dossier->frais_pharmacie) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('medicaments') ? 'is-invalid' : '' }}" name="medicaments[]" id="medicaments" multiple>
                    @foreach($medicaments as $id => $medicament)
                        <option value="{{ $id }}" {{ (in_array($id, old('medicaments', [])) || $dossier->medicaments->contains($id)) ? 'selected' : '' }}>{{ $medicament }}</option>
                    @endforeach
                </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('statut') ? 'is-invalid' : '' }}" name="statut" id="statut">
                        ...
                        @foreach(App\Models\Dossier::STATUT_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('statut', $dossier->statut) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
