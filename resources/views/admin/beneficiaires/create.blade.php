...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.beneficiaire.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.beneficiaires.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('lien_parente') ? 'is-invalid' : '' }}" name="lien_parente" id="lien_parente" required>
                    ...
                    @foreach(App\Models\Beneficiaire::LIEN_PARENTE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('lien_parente', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('statut') ? 'is-invalid' : '' }}" name="statut" id="statut" hidden>
                    ...
                    @foreach(App\Models\Beneficiaire::STATUT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('statut', 'Inactif') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
