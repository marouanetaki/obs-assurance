...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.prise.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.prises.update", [$prise->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('beneficiaire') ? 'is-invalid' : '' }}" name="beneficiaire_id" id="beneficiaire_id" required>
                    @foreach($beneficiaires as $id => $entry)
                        <option value="{{ $id }}" {{ (old('beneficiaire_id') ? old('beneficiaire_id') : $prise->beneficiaire->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('clinique') ? 'is-invalid' : '' }}" name="clinique_id" id="clinique_id" required>
                    @foreach($cliniques as $id => $entry)
                        <option value="{{ $id }}" {{ (old('clinique_id') ? old('clinique_id') : $prise->clinique->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Prise::STATUT_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('statut') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="statut_{{ $key }}" name="statut" value="{{ $key }}" {{ old('statut', $prise->statut) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type_operation') ? 'is-invalid' : '' }}" type="text" name="type_operation" id="type_operation" value="{{ old('type_operation', $prise->type_operation) }}" required>
